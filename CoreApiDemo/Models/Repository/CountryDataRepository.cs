...
namespace CoreApiDemo.Models.Repository
{
    ...
    {
        public IEnumerable<TEntity> GetById(EFDATAContext _EFDATAContext, string ItemCode)
        {
            var result = _EFDATAContext.ViwHsoabirthCountries.Where(a => a.ItemCode.Contains(ItemCode)).Select(a => new ViwHsoabirthCountryDTO {
                NationCode = a.Memo,
